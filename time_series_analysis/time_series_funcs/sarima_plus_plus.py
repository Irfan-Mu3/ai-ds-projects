...
def learn_model(ts, p_pol, q_pol, p_symbs, q_symbs, bounds=(-1, 1)):
    ...
    res = least_squares(calculateSeasonalARIMA_error_minimization_form_slow, bounds=bounds, x0=initial_guess,
...
def batch_stepwise_forecastSeasonalARIMA(y_t, h, p_poly, d_poly, q_poly, y_t_new, num_samples=100,
                                         use_a_t_kde=False):
    # samples sets of forecasts for each h interval.

    num_sets = int(len(y_t_new) / h)
    orig_len = len(y_t)
    y_t = np.append(y_t, y_t_new)

    z_ts = np.empty((num_sets + 1, num_samples, h))
    for i in range(0, num_sets + 1):
        # remember: errors are slightly different depending on y_t length, hence a_t is left inside the for loop:
        a_t = calculateSeasonalARIMA_error(y_t[:orig_len + (h * i)], p_poly, d_poly, q_poly)
        a_t_dist = None if not use_a_t_kde else gaussian_kde(a_t).pdf

        for j in range(num_samples):
            z_t = forecastSeasonalARIMA(y_t[:orig_len + (h * i)], a_t, h, p_poly, d_poly, q_poly, sample_errors=True,
                                        a_t_dist=a_t_dist)
            z_ts[i, j] = z_t[- h:]  # just take the last h timesteps.

    return z_ts, np.std(z_ts, axis=1), np.mean(z_ts, axis=1)


def stepwise_forecastSeasonalARIMA(y_t, h, p_poly, d_poly, q_poly, y_t_new, sample_errors=False, use_a_t_kde=False):
    # Uses expected value to forecast for each h interval.
    ...
    for i in range(0, num_sets + 1):
        a_t = calculateSeasonalARIMA_error(y_t[:orig_len + (h * i)], p_poly, d_poly, q_poly)
        a_t_dist = None if not use_a_t_kde else gaussian_kde(a_t).pdf
        z_t = forecastSeasonalARIMA(y_t[:orig_len + (h * i)], a_t, h, p_poly, d_poly, q_poly,
                                    sample_errors=sample_errors, a_t_dist=a_t_dist)
...
def batch_forecastSeasonalARIMA(y_t, a_t, h, p_poly, d_poly, q_poly, num_samples=100, a_t_dist=None):
    z_ts = np.empty((num_samples, len(a_t) + h))

    for i in range(num_samples):
        z_t = forecastSeasonalARIMA(y_t, a_t, h, p_poly, d_poly, q_poly, True, a_t_dist)
        z_ts[i] = z_t

    return z_ts, np.std(z_ts, axis=0), np.mean(z_ts, axis=1)


def forecastSeasonalARIMA(y_t, a_t, h, p_poly, d_poly, q_poly, sample_errors=False, a_t_dist=None):
    if sample_errors:
        if a_t_dist is None:
            a_t = np.append(a_t, np.random.normal(0, np.std(a_t), h))  # remember: uses the variance of the error terms
        else:
            a_t = np.append(a_t, a_t_dist(h))
    else:
    ...
    # step Integration filter
    z_t = integrate_series(w_t, d_poly, True)
...
def lambdify_poly_coeff_creator(poly_coeffs, symbols):
    # the symbols need not be ordered, however poly_coeffs must be with the furthest lag first (for learning purposes),
...
def compute_conf_intervals_via_shock_poly(a_t, p_poly, d_poly, q_poly, num_coeffs, max_lag=100):
    ...
    shock_poly: poly = (series(q_poly / (d_poly * p_poly), x=B, x0=0, n=max_lag + 1)).as_expr().removeO().as_poly()
...
if __name__ == '__main__':
    ...
    max_lag = 100
    error_bound = compute_conf_intervals_via_shock_poly(a_t_error_2p0, est_p_poly, d_poly, est_q_poly, num_coeffs,
                                                        max_lag=max_lag)
